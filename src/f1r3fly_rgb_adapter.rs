...
use lightning::rgb_utils::{
    ...
    RgbLibError, RgbLibTransaction, RgbTransfer, SettlementExecutor, SignOptions, Transfer,
    Unspent, UpdateRes, WitnessOrd,
};
...
impl F1r3flyRgbWalletWrapper {
    ...
    pub async fn new(wallet_data: wallet::WalletData) -> Result<Self, RgbLibError> {
        ...
        if load_result.is_err() {
            tracing::info!("Wallet doesn't exist, importing from mnemonic");
            ...
            tracing::info!("Wallet imported and loaded");
        } else {
            tracing::info!("Existing wallet loaded");
        }
    ...
    pub fn issue_asset_nia(
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                mgr.sync_wallet().await.map_err(|e| {
                    RgbLibError::Other(format!("Sync before issuance failed: {}", e))
                })?;
                ...
                let genesis_utxo = utxos.first().ok_or(RgbLibError::InsufficientBitcoins {
                    needed: 1000,
                    available: 0,
                })?;
                ...
                // Issue the asset
                let asset_info =
                    mgr.issue_asset(request)
                        .await
                        .map_err(|e| RgbLibError::FailedIssuance {
                            details: format!("F1r3fly issuance failed: {}", e),
                        })?;

                tracing::info!(contract_id = %asset_info.contract_id, "RGB asset issued");
    ...
    pub fn list_assets(
        &self,
        _filter_asset_schemas: Vec<AssetSchema>,
            ...
            futures::executor::block_on(async {
                ...
                for asset_item in assets {
                    ...
                    let (asset_info, balance_info) = {
                        ...
                        let asset_info =
                            mgr.get_asset_info(&asset_item.contract_id).map_err(|e| {
                                RgbLibError::Other(format!("Failed to get asset info: {}", e))
                            })?;
                    ...
                    let locked_amount =
                        self.get_locked_amount(&asset_item.contract_id).unwrap_or(0); // If error, assume 0 locked
    ...
    pub fn create_utxos(
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                let occupied_utxos = mgr.get_occupied_utxos().await.map_err(|e| {
                    RgbLibError::Other(format!("Failed to get occupied UTXOs: {}", e))
                })?;
                ...
                for _ in 0..count {
                    let address_info = bitcoin_wallet
                        .inner_mut()
                ...
                for rgb_utxo in &occupied_utxos {
                    ...
                    if parts.len() != 2 {
                        tracing::warn!(outpoint = %rgb_utxo.outpoint, "Invalid outpoint format, skipping");
                        continue;
                    }

                    let txid = bdk_wallet::bitcoin::Txid::from_str(parts[0]).map_err(|e| {
                        RgbLibError::Other(format!("Invalid txid in {}: {}", rgb_utxo.outpoint, e))
                    })?;
                    let vout: u32 = parts[1].parse().map_err(|e| {
                        RgbLibError::Other(format!("Invalid vout in {}: {}", rgb_utxo.outpoint, e))
                    })?;
                ...
                let mut psbt = tx_builder.finish().map_err(|e| {
                    RgbLibError::Other(format!("Failed to build split transaction: {}", e))
                })?;
                ...
                bitcoin_wallet
                    ...
                    .map_err(|e| {
                        RgbLibError::Other(format!("Failed to sign transaction: {}", e))
                    })?;

                // Extract and broadcast the transaction
                let tx = psbt.extract_tx().map_err(|e| {
                    RgbLibError::Other(format!("Failed to extract transaction: {}", e))
                })?;
                ...
                esplora_client.inner().broadcast(&tx).map_err(|e| {
                    RgbLibError::Other(format!("Failed to broadcast transaction: {}", e))
                })?;
                ...
                let mut mgr = manager.lock().unwrap();
                mgr.sync_wallet().await.map_err(|e| {
                    RgbLibError::Other(format!("Sync after createutxos failed: {}", e))
                })?;
    ...
    pub fn blind_receive(
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                Err(RgbLibError::Other(
                    "blind_receive: Not fully implemented".to_string(),
                ))
            })
        })
    }
    ...
    pub fn send(
        ...
        let rgb_amount = match &recipient.assignment {
            ...
            _ => {
                return Err(RgbLibError::Other(
                    "Non-fungible assignments not supported".to_string(),
                ))
            }
        };
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                let rgb_occupied_vec = mgr.get_occupied_utxos().await.map_err(|e| {
                    RgbLibError::Other(format!("Failed to get occupied UTXOs: {}", e))
                })?;

                let state_hash = mgr
                    .get_genesis_state_hash(&asset_id_clone)
                ...
                tx_builder.fee_rate(bdk_wallet::bitcoin::FeeRate::from_sat_per_vb_unchecked(
                    fee_rate,
                ));
                ...
                for occupied_utxo in rgb_occupied_vec {
                    let outpoint =
                        bitcoin::OutPoint::from_str(&occupied_utxo.outpoint).map_err(|e| {
                            RgbLibError::Other(format!(
                                "Invalid outpoint {}: {}",
                                occupied_utxo.outpoint, e
                            ))
                        })?;
                    tx_builder.add_unspendable(outpoint);
                }
                ...
                if !finalized {
                    return Err(RgbLibError::Other(
                        "PSBT signing did not finalize".to_string(),
                    ));
                }
                ...
                let state: serde_json::Value = serde_json::from_str(&state_json).map_err(|e| {
                    RgbLibError::Other(format!("Failed to parse f1r3fly_state.json: {}", e))
                })?;

                let genesis_utxo_data = state["genesis_utxos"][&asset_id_clone].as_object().ok_or(
                    RgbLibError::Other(format!(
                        "Contract {} not found in genesis_utxos",
                        asset_id_clone
                    )),
                )?;

                let genesis_txid = genesis_utxo_data["txid"]
                    .as_str()
                    .ok_or(RgbLibError::Other(
                        "Missing txid in genesis UTXO".to_string(),
                    ))?;
                let genesis_vout = genesis_utxo_data["vout"]
                    .as_u64()
                    .ok_or(RgbLibError::Other(
                        "Missing vout in genesis UTXO".to_string(),
                    ))?;
                ...
                {
                    let contracts_mgr = mgr.f1r3fly_contracts_mut().ok_or(RgbLibError::Other(
                    ...
                    let derivation_index = contracts_mgr
                        .get_contract_derivation_index(&asset_id_clone)
                        .map_err(|e| {
                            RgbLibError::Other(format!("Failed to get derivation index: {}", e))
                        })?;
                    let signing_key = contracts_mgr
                        ...
                        .map_err(|e| {
                            RgbLibError::Other(format!("Failed to get signing key: {}", e))
                        })?;
                    ...
                    .map_err(|e| {
                        RgbLibError::Other(format!("Failed to generate transfer signature: {}", e))
                    })?;
                    ...
                    let _result = contract
                        ...
                        .await
                        .map_err(|e| {
                            RgbLibError::Other(format!("F1r3node transfer failed: {}", e))
                        })?;
                ...
                let broadcast_txid = mgr
                    .broadcast_transaction(&tx)
                    .map_err(|e| RgbLibError::Other(format!("Failed to broadcast: {}", e)))?;

                tracing::info!(txid = %broadcast_txid, "RGB transfer broadcast");
                ...
                if !transport_endpoints.is_empty() {
                    ...
                    for endpoint_str in transport_endpoints {
                        ...
                        let json_bytes = serde_json::to_vec(&payload).map_err(|e| {
                            RgbLibError::Other(format!("Failed to serialize payload: {}", e))
                        })?;
                        ...
                        let file_part = reqwest::blocking::multipart::Part::bytes(json_bytes)
                            ...
                            .map_err(|e| {
                                RgbLibError::Other(format!("Failed to create file part: {}", e))
                            })?;
                        ...
                        let response =
                            client.post(&base_url).multipart(form).send().map_err(|e| {
                                RgbLibError::Other(format!("HTTP POST failed: {}", e))
                            })?;

                        if !response.status().is_success() {
                            tracing::warn!(status = %response.status(), "Proxy POST failed");
                        }
                    }
                }
    ...
    pub fn send_begin(
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                let state_hash = mgr
                    .get_genesis_state_hash(asset_id)
                    .map_err(|e| RgbLibError::Other(format!("Failed to get state hash: {}", e)))?;
                ...
                let mut tx_builder = bitcoin_wallet.inner_mut().build_tx();
                tx_builder
                    .add_recipient(addr.script_pubkey(), bitcoin::Amount::from_sat(amount_sat));
                tx_builder.fee_rate(bdk_wallet::bitcoin::FeeRate::from_sat_per_vb_unchecked(
                    fee_rate,
                ));
                ...
                let transfer_details_path =
                    self.data_dir.join(".ldk").join("pending_rgb_transfer.json");
                std::fs::write(
                    &transfer_details_path,
                    serde_json::to_string(&transfer_details).unwrap(),
                )
                .map_err(|e| {
                    RgbLibError::Other(format!("Failed to store transfer details: {}", e))
                })?;
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                let transfer_details_str = std::fs::read_to_string(&transfer_details_path)
                    .map_err(|e| {
                        RgbLibError::Other(format!("Failed to read transfer details: {}", e))
                    })?;
                let transfer_details: serde_json::Value =
                    serde_json::from_str(&transfer_details_str).map_err(|e| {
                        RgbLibError::Other(format!("Failed to parse transfer details: {}", e))
                    })?;

                let contract_id =
                    transfer_details["contract_id"]
                        .as_str()
                        .ok_or(RgbLibError::Other(
                            "Missing contract_id in transfer details".to_string(),
                        ))?;
                let rgb_amount =
                    transfer_details["rgb_amount"]
                        .as_u64()
                        .ok_or(RgbLibError::Other(
                            "Missing rgb_amount in transfer details".to_string(),
                        ))?;
                ...
                let tx = psbt.extract_tx().map_err(|e| {
                    RgbLibError::Other(format!("Failed to extract TX from PSBT: {}", e))
                })?;
                ...
                let (funding_vout, _) = tx
                    ...
                    .ok_or(RgbLibError::Other(
                        "No funding output found in TX".to_string(),
                    ))?;
                ...
                let already_broadcast = match mgr.broadcast_transaction(&tx) {
                    ...
                    Err(e) => {
                        ...
                        if err_msg.contains("already in utxo set")
                            || err_msg.contains("txn-already-in-mempool")
                        {
                            tracing::debug!(txid = %funding_txid, "TX already broadcast, skipping F1r3node transfer");
                ...
                if !already_broadcast {
                    ...
                    let genesis_utxo_data = state["genesis_utxos"][contract_id].as_object().ok_or(
                        RgbLibError::Other(format!(
                            "Contract {} not found in genesis_utxos",
                            contract_id
                        )),
                    ...
                    let genesis_txid =
                        genesis_utxo_data["txid"]
                            .as_str()
                            .ok_or(RgbLibError::Other(
                                "Missing txid in genesis UTXO".to_string(),
                            ))?;
                    let genesis_vout =
                        genesis_utxo_data["vout"]
                            .as_u64()
                            .ok_or(RgbLibError::Other(
                                "Missing vout in genesis UTXO".to_string(),
                            ))?;
                    ...
                    // The contracts_mgr reference will be dropped at the end of this block
                    {
                        ...
                        let contracts_mgr = mgr.f1r3fly_contracts_mut().ok_or(
                            RgbLibError::Other("Contracts manager not initialized".to_string()),
                        )?;
                        ...
                        let derivation_index = contracts_mgr
                            .get_contract_derivation_index(contract_id)
                            .map_err(|e| {
                                RgbLibError::Other(format!("Failed to get derivation index: {}", e))
                            })?;
                        let signing_key = contracts_mgr
                        ...
                        contracts_mgr.save_state().map_err(|e| {
                            RgbLibError::Other(format!(
                                "Failed to save state after transfer: {}",
                                e
                            ))
                        })?;
                    ...
                    .map_err(|e| {
                        RgbLibError::Other(format!("Failed to track locked UTXO: {}", e))
                    })?;
                }
                ...
                Ok(OperationResult { txid: funding_txid })
            })
        })
    }
    ...
    pub fn sign_psbt(
        ...
        _sign_options: Option<SignOptions>,
    ) -> Result<String, RgbLibError> {
        let manager = self.wallet_manager.clone();
    ...
    pub fn color_psbt_and_consume(
        ...
        Err(RgbLibError::Other(
            "color_psbt_and_consume: Not fully implemented".to_string(),
        ))
    }
    ...
    pub fn post_consignment<P: AsRef<Path>>(
        ...
        let _path_str = consignment_path
            .as_ref()
            .to_str()
            .ok_or(RgbLibError::Other("Invalid consignment path".to_string()))?;

        let filename = consignment_path
            .as_ref()
            .file_name()
            .and_then(|f| f.to_str())
        ...
        let manager = self.wallet_manager.clone();
        let (state_hash, asset_info, contract_metadata, wallet_pubkey) =
            tokio::task::block_in_place(|| {
                futures::executor::block_on(async {
                    let mgr = manager.lock().unwrap();

                    // Get genesis state hash
                    let state_hash = mgr.get_genesis_state_hash(&asset_id).map_err(|e| {
                        RgbLibError::Other(format!("Failed to get state hash: {}", e))
                    })?;

                    // Get asset info (ticker, name, precision, supply)
                    let asset_info = mgr.get_asset_info(&asset_id).map_err(|e| {
                        RgbLibError::Other(format!("Failed to get asset info: {}", e))
                    })?;

                    // Get contract metadata (registry_uri, rholang_source)
                    let contract_metadata = mgr.get_contract_metadata(&asset_id).map_err(|e| {
                        RgbLibError::Other(format!("Failed to get contract metadata: {}", e))
                    })?;
                    ...
                    let wallet_pubkey = mgr
                        .f1r3fly_contracts()
                        .ok_or_else(|| {
                            RgbLibError::Other("F1r3fly contracts not initialized".to_string())
                        })?
                        .contracts()
                        .executor()
                        .get_master_public_key_hex()
                        .map_err(|e| {
                            RgbLibError::Other(format!("Failed to get wallet public key: {}", e))
                        })?;
        ...
        let result = std::thread::spawn(move || {
            ...
            let counterparty_pubkey = {
                ...
                if let Some(pubkey) = cached_pubkey {
                    ...
                    tracing::debug!("Counterparty pubkey not in cache, fetching from proxy");
            ...
            let result = rt.block_on(call_future)
                .map_err(|e| RgbLibError::Other(format!("F1r3node settle_channel call failed: {}", e)))?;
    ...
    pub fn claim(
        ...
        let result = std::thread::spawn(move || {
            ...
            let contracts_mgr = mgr.f1r3fly_contracts_mut().ok_or(RgbLibError::Other(
                "F1r3fly contracts not initialized".to_string(),
            ))?;
            ...
            let signing_key = contracts_mgr
                ...
                .get_master_key()
                .map_err(|e| {
                    RgbLibError::Other(format!("Failed to get wallet master key: {}", e))
                })?;
            ...
            let call_future = contract.call_method("claim", &params, SmallOrdMap::new());

            // Create a new tokio runtime for this thread to execute the async call
            let rt = tokio::runtime::Runtime::new().map_err(|e| {
                RgbLibError::Other(format!("Failed to create tokio runtime: {}", e))
            })?;

            let _result = rt
                .block_on(call_future)
                .map_err(|e| RgbLibError::Other(format!("F1r3node claim call failed: {}", e)))?;
            ...
            // Aligns with f1r3fly-rgb-wallet's claim tracking mechanism
            {
                ...
                let destination_vout: u32 = parts[1]
                    .parse()
                ...
                if !already_claimed {
                    ...
                    let claim_id = contracts_mgr
                        ...
                        .map_err(|e| {
                            RgbLibError::Other(format!("Failed to insert claim: {}", e))
                        })?;
                    ...
                    contracts_mgr
                        ...
                        .map_err(|e| {
                            RgbLibError::Other(format!("Failed to mark claim completed: {}", e))
                        })?;
                }
            }
            ...
            contracts_mgr.save_state().map_err(|e| {
                RgbLibError::Other(format!("Failed to save state after claim: {}", e))
            })?;

            Ok(())
        })
        .join()
        .map_err(|e| RgbLibError::Other(format!("Thread join failed: {:?}", e)))??;
    ...
    pub fn get_claim_utxo(&self) -> Result<String, RgbLibError> {
        ...
        if let Some(utxo) = unspents
            ...
            .filter(|u| u.rgb_allocations.is_empty()) // Not RGB-occupied
            .filter(|u| u.utxo.btc_amount <= 2000) // Small (claim-sized)
            .min_by_key(|u| u.utxo.btc_amount)
        // Smallest first
        {
        ...
        let unspents = self.list_unspents(None, false, false)?;
        let utxo = unspents.last().ok_or(RgbLibError::Other(
            "Failed to create claim UTXO".to_string(),
        ))?;
    ...
    fn track_locked_utxo(
        ...
        let locked_file_path = self
            .data_dir
            .join("rgb-lightning-wallet")
            .join("locked_channel_utxos.json");
    ...
    pub fn release_locked_utxo(&self, txid: &str, vout: u32) -> Result<(), String> {
        ...
        let content = std::fs::read_to_string(&locked_file_path)
            .map_err(|e| format!("Failed to read locked UTXOs file: {}", e))?;
        let mut locked_data: serde_json::Value =
            serde_json::from_str(&content).unwrap_or_else(|_| serde_json::json!({}));
    ...
    fn get_locked_amount(&self, contract_id: &str) -> Result<u64, String> {
        ...
        let content = std::fs::read_to_string(&locked_file_path)
            .map_err(|e| format!("Failed to read locked UTXOs file: {}", e))?;
        let locked_data: serde_json::Value =
            serde_json::from_str(&content).unwrap_or_else(|_| serde_json::json!({}));
        ...
        if let Some(obj) = locked_data.as_object() {
            for (_, utxo_data) in obj {
                if let Some(utxo_contract_id) =
                    utxo_data.get("contract_id").and_then(|c| c.as_str())
                {
    ...
    #[allow(dead_code)]
    fn read_rgb_channel_info_from_txid(
        &self,
        ldk_data_dir: &Path,
        _txid: &str,
    ) -> Result<lightning::rgb_utils::RgbInfo, RgbLibError> {
        ...
        for entry in entries {
            let entry =
                entry.map_err(|e| RgbLibError::Other(format!("Cannot read entry: {}", e)))?;
            let path = entry.path();

            if let Some(filename) = path.file_name().and_then(|f| f.to_str()) {
                if filename.ends_with("_pending")
                    && !filename.starts_with('.')
                    && !filename.contains("consignment")
                {
                    // Try to read this file
                    if let Ok(content) = fs::read_to_string(&path) {
                        if let Ok(rgb_info) =
                            serde_json::from_str::<lightning::rgb_utils::RgbInfo>(&content)
                        {
        ...
        Err(RgbLibError::Other(
            "Cannot find RGB channel info - channel may not be RGB-enabled".to_string(),
        ))
    }
...
impl lightning::events::bump_transaction::WalletSource for F1r3flyRgbWalletWrapper {
    ...
    fn sign_psbt(&self, psbt: bitcoin::psbt::Psbt) -> Result<bitcoin::Transaction, ()> {
        let signed_psbt_str =
            F1r3flyRgbWalletWrapper::sign_psbt(self, psbt.to_string(), None).map_err(|_| ())?;
    ...
    fn is_witness_claimed(&self, witness_id: &str, contract_id: &str) -> Result<bool, RgbLibError> {
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                let is_claimed = all_claims
                    .iter()
                    .any(|claim| claim.witness_id == witness_id);
    ...
    fn reload_contracts(&self) -> Result<(), RgbLibError> {
        ...
        tokio::task::block_in_place(|| {
            futures::executor::block_on(async {
                ...
                mgr.reload_contracts().map_err(|e| {
                    RgbLibError::Other(format!("Failed to reload contracts: {}", e))
                })?;
